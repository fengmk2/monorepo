...
Default headers are merged with request headers.
Request headers win when the same header is provided twice.
...
Factory query params, URL query params, and request query params are merged in that order.
Later values win.

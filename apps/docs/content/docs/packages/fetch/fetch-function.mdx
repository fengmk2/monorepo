...
Call `$fetch(resource, options)` without a schema to receive the native `Response`.
...
Call `$fetch(resource, schema, options)` to parse `response.json()` and validate the parsed value.
...
Validation output is inferred from the schema.
The schema validates the response body, not request `json` or `body` options.

## Request Inputs

The `resource` argument follows native `fetch` and accepts `RequestInfo`.

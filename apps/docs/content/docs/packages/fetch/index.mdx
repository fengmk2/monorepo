...
TypeScript can describe the data you expect from an API, but it cannot prove the API actually returned that shape.
Without runtime validation, fetch code often ends with unchecked assertions.
...
- **Raw fetch mode** through `$fetch(resource, options)`.
- **Validated fetch mode** through `$fetch(resource, schema, options)`.
